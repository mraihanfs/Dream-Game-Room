<!DOCTYPE html>
<?php
    $nilai=$_GET['submit'];
    session_start();
    if (isset($_SESSION['name'])){

    }else{
        echo "<script>alert('Anda Belum Login silahkan login terlebih dahulu !!!!'); document.location= './Login/Login_User.html'</script>";
    }
?>
<html>

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
    <title>Booking - User</title>
    <link rel="stylesheet" href="assets/bootstrap/css/bootstrap.min.css">
    <link rel="stylesheet"
        href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i">
    <link rel="stylesheet" href="assets/fonts/fontawesome-all.min.css">
    <link rel="stylesheet" href="/assets/bootstrap/css/style.css">
</head>

<body id="page-top">
    <div id="wrapper">
        <nav
            class="navbar navbar-dark bg-dark align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0">
            <div class="container-fluid d-flex flex-column p-0">
                <a class="navbar-brand d-flex justify-content-center align-items-center sidebar-brand m-0" href="index.html">
                    <div class="sidebar-brand-icon rotate-n-15"><i class="fas fa-laugh-wink"></i></div>
                    <div class="sidebar-brand-text mx-3"><span>Dream Game
                            <br>Room</span></div>
                </a>
                <hr class="sidebar-divider my-0">
                <ul class="nav navbar-nav text-light" id="accordionSidebar">
                    <li class="nav-item" role="presentation"><a class="nav-link text-dark active"
                            href="dashboard.php"><i class="fas fa-tachometer-alt"></i><span>Dashboard</span></a></li>
                    <li class="nav-item" role="presentation"><a class="nav-link" href="my_Booking.php"><i
                                class="fas fa-table"></i><span>My Booking</span></a></li>
                </ul>
                <div class="text-center d-none d-md-inline"><button class="btn rounded-circle border-0"
                        id="sidebarToggle" type="button"></button></div>
            </div>
        </nav>
        <div class="d-flex flex-column" id="content-wrapper">
            <div id="content">
                <nav class="navbar navbar-light navbar-expand bg-dark shadow mb-4 topbar static-top">
                    <div class="container-fluid"><button class="btn btn-link d-md-none rounded-circle mr-3"
                            id="sidebarToggleTop" type="button"><i class="fas fa-bars"></i></button>
                        <ul class="nav navbar-nav flex-nowrap ml-auto">
                            <div class="d-none d-sm-block topbar-divider"></div>
                            <li class="nav-item dropdown no-arrow" role="presentation">
                                <div class="nav-item dropdown no-arrow"><a class="dropdown-toggle nav-link"
                                        data-toggle="dropdown" aria-expanded="false" href="#"><span
                                            class="d-none d-lg-inline mr-2 text-gray-600 small"><?php echo $_SESSION["name"];?></span><img
                                            class="border rounded-circle img-profile" src="assets/img/iconorg.png"></a>
                                    <div class="dropdown-menu shadow dropdown-menu-right animated--grow-in" role="menu">
                                        <a class="dropdown-item" role="presentation" href="#"><i
                                                class="fas fa-user fa-sm fa-fw mr-2 text-gray-400"></i>&nbsp;Profile</a><a
                                            class="dropdown-item" role="presentation" href="#"><i
                                                class="fas fa-cogs fa-sm fa-fw mr-2 text-gray-400"></i>&nbsp;Settings</a>
                                        <div class="dropdown-divider"></div><a class="dropdown-item" role="presentation"
                                            href="index.php"><i
                                                class="fas fa-sign-out-alt fa-sm fa-fw mr-2 text-gray-400"></i>&nbsp;Logout</a>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                </nav>
                <div class="container-fluid">
                    <h3 class="text-dark mb-4">Booking</h3>
                    <div class="row mb-3">
                        <div class="col-lg-4">
                            <div class="card mb-3">
                                <div class="card-body text-center shadow ">
                                    <div class="pricingTable">
                                        <h1 class="title">PC-<?php echo $nilai ?></h1>
                                        <img class=" mb-3 mt-4" src="assets/img/incon2.png" width="160" height="160">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-8">
                            <!-- PC Info sekilas nya-->
                            <div class="row">
                                <div class="col">
                                    <div class="card shadow mb-3">
                                        <div class="card-header py-3">
                                            <p class="text-primary m-0 font-weight-bold">Info</p>
                                        </div>
                                        <div class="card-body">
                                            <form>
                                                <div class="form-row">
                                                    <div class="col">
                                                        <div class="form-group">
                                                            <strong>Letak :</strong>
                                                        </div>
                                                    </div>
                                                    <div class="col">
                                                        <div class="form-group">
                                                            <p> Lantai 10 Blok E</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="form-row">
                                                    <div class="col">
                                                        <div class="form-group">
                                                            <strong>Kondisi :</strong>
                                                        </div>
                                                    </div>
                                                    <div class="col">
                                                        <div class="form-group">
                                                            <p>Baik</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="form-row">
                                                    <div class="col">
                                                        <div class="form-group">
                                                            <strong>Harga :</strong>
                                                        </div>
                                                    </div>
                                                    <div class="col">
                                                        <div class="form-group">
                                                            <p>Rp. 10.000/Jam</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                                    <!-- Spesifikasi Detail PC-->
                                    <div class="card shadow">
                                        <div class="card-header py-3">
                                            <p class="text-primary m-0 font-weight-bold">Spesifikasi Detail</p>
                                        </div>
                                        <div class="card-body">
                                            
                                                <div class="form-group">
                                                    <strong>Processor
                                                    </strong>
                                                    <ul class="oy">
                                                        <li class="dropdown">

                                                            <ul class="isi-dropdown">
                                                                <li>Intel® Core™ i7-9750H </li>
                                                            </ul>
                                                        </li>
                                                    </ul>
                                                </div>

                                                <div class="form-group">
                                                    <strong>Graphics
                                                    </strong>
                                                    <ul class="oy">
                                                        <li class="dropdown">
                                                            >
                                                            <ul class="isi-dropdown">
                                                                <li>NVIDIA® GeForce RTX™ 2080 (Max-Q)(Boost Clock:
                                                                    1230MHz, 90W)
                                                                    8GB GDDR6 VRAM</li>
                                                            </ul>
                                                        </li>
                                                    </ul>
                                                </div>

                                                <div class="form-group">
                                                    <strong>Ram
                                                    </strong>
                                                    <ul class="oy">
                                                        <li class="dropdown">
                                                            >
                                                            <ul class="isi-dropdown">
                                                                <li>8 GB (DDR4 2666MHz SDRAM)</li>
                                                            </ul>
                                                        </li>
                                                    </ul>
                                                </div>

                                                <div class="form-group">
                                                    <strong>Keyboard
                                                    </strong>
                                                    <ul class="oy">
                                                        <li class="dropdown">
                                                            >
                                                            <ul class="isi-dropdown">
                                                                <li>Backlit Chiclet Keyboard ,Marked WASD keys
                                                                    <br>
                                                                    N key rollover, RGB 4 zones
                                                                    <br>
                                                                    Aura Sync, 1.2mm travel distance</li>
                                                            </ul>
                                                        </li>
                                                    </ul>
                                                </div>

                                                <div class="form-group">
                                                    <strong>Audio
                                                    </strong>
                                                    <ul class="oy">
                                                        <li class="dropdown">
                                                            >
                                                            <ul class="isi-dropdown">
                                                                <li>2x 2W speakers with Smart AMP technology Array
                                                                    Microphone</li>
                                                            </ul>
                                                        </li>
                                                    </ul>
                                                </div>

                                                <div class="form-row">
                                                    <div class="col">
                                                        <div class="form-group">
                                                            <a href="./dashboard.php" class="btn btn-primary btn-sm">Kembali</a>
                                                        </div>
                                                    </div>
                                                    <div class="col4">
                                                        <div class="form-group">
                                                            <button type="button" class="btn btn-primary  btn-sm"
                                                                data-toggle="modal"
                                                                data-target="#myModal">Booking</button>
                                                        </div>
                                                    </div>
                                                </div>

                                                <div id="myModal" class="modal fade" role="dialog">
                                                    <div class="modal-dialog">
                                                        <!-- konten modal-->
                                                        <div class="modal-content">
                                                            <!-- heading modal -->
                                                            <div class="modal-header">
                                                                <h4 class="modal-title">Konfirmasi Booking</h4>
                                                                <button type="button" class="close"
                                                                    data-dismiss="modal">&times;</button>
                                                            </div>
                                                            <!-- body modal -->
                                                            <div class="modal-body">
                                                                <p>Apakah Anda yakin Ingin membooking nya ???</p>
                                                            </div>
                                                            <!-- footer modal -->
                                                            <div class="modal-footer">
                                                                <div class="col">
                                                                    <button type="button" class="btn btn-default"
                                                                        data-dismiss="modal">Tidak</button>
                                                                </div>
                                                                <div class="col4">
                                                                    <form action="./bookingP.php" method="POST">
                                                                        <button type="submit" class="btn btn-primary shadow" value="<?php echo $nilai?>" name="submit" >Ya!</button>
                                                                    </form>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>

                                            
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <footer class="bg-dark sticky-footer">
                <div class="container my-auto">
                    <div class="text-center my-auto copyright bg-dark"><span>Copyright © DREAM TEAM 2020</span>
                    </div>
                </div>
            </footer>
        </div><a class="border rounded d-inline scroll-to-top" href="#page-top"><i class="fas fa-angle-up"></i></a>
    </div>
    <script src="assets/js/jquery.min.js"></script>
    <script src="assets/bootstrap/js/bootstrap.min.js"></script>
    <script src="assets/js/chart.min.js"></script>
    <script src="assets/js/bs-init.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery-easing/1.4.1/jquery.easing.js"></script>
    <script src="assets/js/theme.js"></script>
</body>

</html>